...
class Main extends PluginBase implements Listener {
    protected function onEnable() : void {
        ...
        if (!DialogueEntity::register()) {
            $this->getLogger()->warning("Without the Customies plugin, player will only be able to see messages in plain text.");
        }
        ...
        foreach ($this->getResources() as $path => $file) {
            ...
            foreach ($data["codes"] as $code) {
                $this->messages[$code] = new DialogueMessages(
    ...
    public function onPlayerMove(PlayerMoveEvent $event) : void {
        $player = $event->getPlayer();
        if (!isset($this->movementLocks[$player->getId()])) {
            return;
        }
